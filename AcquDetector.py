...
def Calibrate(adcArray,detlist=[]):
    ...
    for detector in detlist:
        ...
        for i, equation, param in zip(range(len(detequ[detector])),detequ[detector],detparams[detector]):
            ...
            sortsort = np.argsort(sort[filt[filt!=-1]])
                        
            rawValues               = CollapseArray(adcArray[(filt+1).astype(np.bool)][sortsort])
            ...
            if(len(detref[detector][i])):
                ...
                refs = adcArray[filt]['val'].astype(np.float)
                ####BAD HACK####
...
def CollapseArray(array):
    ...
    valueCol    = np.split(array['val'].astype(np.int),sumIndeces)
    return valueCol
